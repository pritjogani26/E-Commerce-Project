...
function updateTotal() {
    ...
    cartItems.forEach(product => {
        total = 0;
        // let title = product.querySelector('.cart-food-title').value;
        // document.getElementById('item').value = title;
        let priceElement = product.querySelector('.cart-price');
        let price = parseFloat(priceElement.innerHTML.replace("Rs.", ""));
        document.getElementById('aprice').value = price
        let qty = product.querySelector('.cart-quantity').value;
        document.getElementById('qun').value = qty;
        total += (price * qty);
...
let idols = document.getElementById('idol');
console.log(idols);

idols.addEventListener("click", function() {
    console.log("Hello I have Idols");
});